// ✅ Updated index.js - Supports both SPListItems and LPItems
...
exports.handler = async (event) => {
    ...
    // Support both single item and bulk operations
    ...
    try {
        if (body.items && Array.isArray(body.items)) {
            ...
            return await handleSingleItem(body, tableName);
        }
    } catch (err) {
...
async function handleSingleItem(body, tableName) {
    let item;
    ...
    if (tableName === "LPItems") {
        ...
        // SPListItems (Filter Reconciliation) structure - backward compatibility
        item = {
    ...
    return {
        statusCode: 200,
        body: JSON.stringify({
            message: `Item saved to ${tableName}`,
            itemId: item.id
        }),
    };
}
...
async function handleBulkOperation(items, tableName, source) {
    console.log(`Processing ${items.length} items for table: ${tableName}`);
    ...
    const processedItems = items.map((item, index) => {
        if (tableName === "LPItems") {
            ...
            return {
                ...
                wardDept: item['Ward/Dept'] || item.wardDept || '',
            ...
            // SPListItems structure for backward compatibility
